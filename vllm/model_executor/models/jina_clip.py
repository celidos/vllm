...
#         logger.info(f'***************************************************Entering config')
#         logger.info(f'\n\n')
        
#         logger.info(f' CONFIG TRACEBACK:\n ')
#         traceback.print_exc()
#         logger.info(f'\n\n **************************************')
#         logger.info(f' ------------ Getting inside jinaclip, we got text_config={text_config}')
...
class JinaCLIPModel(JinaCLIPPreTrainedModel):
    ...
    def __init__(
        ...
        logger.info(f'VLLM CONFIG\n\n{vllm_config}')

        logger.info(f'\n\nASSSSS {type(vllm_config.model_config)} {vllm_config.model_config}')

        logger.info(f'\n\nASzzzzzzz {type(vllm_config.model_config.hf_config)} {vllm_config.model_config.hf_config}')
        
        
        # read_config = PretrainedConfig.from_pretrained(vllm_config.model_config.model)
        # logger.info(f'PAssing dict to JinaCLIP CONFIG: {read_config.to_dict()}')
        # config = JinaCLIPConfig(**(read_config.to_dict()))
        
        # config = PretrainedConfig.from_pretrained(vllm_config.model_config.model)

        # config = JinaCLIPConfig.from_pretrained(vllm_config.model_config.model)

        # read_config = PretrainedConfig.from_pretrained(vllm_config.model_config.model)
        # logger.info(f'PAssing dict to JinaCLIP CONFIG: {read_config.to_dict()}')
        # read_config_dict = read_config.to_dict()
        # logger.info(f'Done DICTING _____________ {read_config_dict}')
        # config = JinaCLIPConfig(**read_config_dict)
        config = vllm_config.model_config.hf_config#JinaCLIPConfig(**vllm_config.model_config.hf_config)
        ...
        logger.info(f'TEXT CONFIG *** = {text_config}')
    ...
    def forward(  # ME: new forward, for now supports only text input
        ...
        **kwargs,
        ...
        logger.info(f'MADE TO OUTPUT SHAPE: {text_embeds.shape}')  # MEX!
